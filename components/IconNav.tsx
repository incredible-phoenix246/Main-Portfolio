...
function IconNav() {
  ...
  return (
    <motion.nav
      ...
      className={cn(
        "flex flex-col items-center xl:justify-center gap-y-4 fixed h-max mt-auto xl:left-[2%] z-50 w-full xl:w-16 xl:max-w-md xl:h-screen bottom-0"
      )}
    >
      <motion.div className="flex w-full xl:flex-col items-center justify-between xl:justify-center gap-y-10 px-4 md:px-40 xl:px-0 h-[80px] xl:h-max py-8 backdrop-blur-sm text-3xl xl:text-xl bg-AAtertiary xl:rounded-full">
        {navData.map((link) => {
          return (
            <Link
              key={link.id}
